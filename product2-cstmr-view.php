...
if (isset($_SESSION['admin']['id'])) {
	if ($_SESSION['admin']['authority'] === 'rw' || $_SESSION['admin']['authority'] === 'r') {
		//権限がrwのみ登録、削除、編集及びsale、rank情報ボタン表示
		if ($_SESSION['admin']['authority'] === 'rw') {
			echo '<button type="submit" name = "button" value="add" form="id1">商品追加</button>';
			echo '<button type="submit" name = "button" value="sale" form="id2">sale情報</button>';
		...
		if ($count !== "zero") {
			#商品一覧をテキストボックスで表示
			echo '<table>';
			echo '<th><a href="product2-cstmr-view.php?order=id">商品id</a></th>
		<th>おすすめ</th><th>商品</th><th>商品名</th><th>価格</th>
		<th><img src="image/sale.png" alt="saleアイコン"></img>セール価格</th>
		<th><a href="product2-cstmr-view.php?order=daily_ranking">昨日ランク</a></th>
		<th><a href="product2-cstmr-view.php?order=weekly_ranking">7日間ランク</a></th>';
